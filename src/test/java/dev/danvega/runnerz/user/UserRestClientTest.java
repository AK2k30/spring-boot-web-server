...
class UserRestClientTest {
  ...
  void shouldFindAllUsers() throws JsonProcessingException {
    ...
    User user1 =
        new User(
            ...
            "lgraham@gmail.com",
            new Address(
                "Kulas Light", "Apt. 556", "Gwenborough", "92998-3874", new Geo(-37.3159, 81.1496)),
    ...
    // when
    this.server
        .expect(requestTo("https://jsonplaceholder.typicode.com/users"))
        .andRespond(
            withSuccess(objectMapper.writeValueAsString(users), MediaType.APPLICATION_JSON));
  ...
  void shouldFindUserById() throws JsonProcessingException {
    ...
    // when
    this.server
        .expect(requestTo("https://jsonplaceholder.typicode.com/users/1"))
