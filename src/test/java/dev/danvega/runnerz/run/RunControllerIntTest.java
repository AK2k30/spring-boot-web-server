...
class RunControllerIntTest {
  ...
  void shouldFindAllRuns() {
    List<Run> runs =
        restClient.get().uri("/api/runs").retrieve().body(new ParameterizedTypeReference<>() {});
  ...
  void shouldCreateNewRun() {
    ...
    ResponseEntity<Void> newRun =
        restClient.post().uri("/api/runs").body(run).retrieve().toBodilessEntity();
  ...
  void shouldUpdateExistingRun() {
    ...
    ResponseEntity<Void> updatedRun =
        restClient.put().uri("/api/runs/1").body(run).retrieve().toBodilessEntity();
